...
export default {
  ...
  methods: {
    ...
    closeAllSwipers () {
      this.ids.forEach(id => this.swiper(id).slidePrev())
    },
    closeAllSwipersExcept (exceptId) {
      this.ids.filter(id => id !== exceptId).forEach(id => this.swiper(id).slidePrev())
    },
    swiperOptions (id) {
      return {
        initialSlide: 0,
